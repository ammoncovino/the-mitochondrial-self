...
Consciousness is not a separate ingredient poured into that system from outside. It is what that system sounds like from the inside, once it gets complicated enough to notice itself operating. The mitochondria were always having the experience — sensing conditions, responding, adjusting, surviving. What changed, in one branch of the tree, is that a nervous system evolved capable of reflecting on that experience and putting it into words. Every other branch is still having it without the reflection. We are having it with the reflection. That is the entire difference. Not a difference in kind. A difference in whether the system talks to itself about what it's doing.

Here is the part that gets missed: once a system can reflect on its own operation and talk about it, it can also get the naming wrong. That is exactly what happened. The experience was real. The name was not. We called it soul. We called it a god watching from outside. We called some tribes chosen and others not, based on nothing more than which name for the same underlying experience they happened to inherit. None of those names described anything that was actually occurring. They were placeholders — the best available explanation from the one part of the system old enough to talk, at a point before there was a biology detailed enough to offer a better one.
...
The clearest way to picture this is the one already sitting inside the biology: evolution behaves like a game that has run for four billion years, where the character never dies, only changes equipment. A fin becomes a limb becomes a hand. A gill becomes a lung. A patch of light-sensitive skin becomes an eye. None of this requires abandoning the earlier version — the new trait bolts onto the old chassis, the way a player picks up a better weapon or a heavier suit of armor without becoming a different character in the process. The chassis itself — the cell, and everything running inside it — is the part of the game that was never rebuilt from scratch. It has simply been carried forward, generation after generation, competing against other branches of the same system while remaining, underneath the surface, the same basic machine.
...
Put the two facts together and the mechanism stops being mysterious. Something is heard, or seen, or remembered. Dopamine releases. The signal doesn't stay contained in the skull — it reaches the heart's own dense neural and mitochondrial network, and the gut's, each processing the same surge locally, on their own wiring, at their own tempo. That is why a powerful moment doesn't just register as a thought. It registers as a physical event in a specific place: a chest that tightens, a gut that drops or turns over, a feeling that seems to arrive from somewhere other than the part of you doing the thinking. It feels like a second place because it is a second place — one with its own nerve net and its own energy system, reacting on a shared signal a fraction of a second out of sync with the head. What gets called a spiritual awakening, felt in the chest or the gut rather than reasoned out in the head, is this: three densely mitochondrial, independently wired networks responding to the same dopamine surge, slightly out of step with each other, at once.
